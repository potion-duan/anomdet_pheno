...
void processGenParticle(const GenParticle* genparticle, int particleIdx, EventData& data, TClonesArray* branchParticle, 
                       int& higgs_count, TLorentzVector& higgs1_p4, TLorentzVector& higgs2_p4, TLorentzVector& higgs3_p4,
    ...
    if(((std::abs(genparticle->PID) == 25)||(std::abs(genparticle->PID) == 35)) && genparticle->Status == 22) {
        ...
        if(higgs_count == 0) {
            ...
            // Now that we have three Higgs, calculate trihiggs variables
        ...
        // Ignore additional Higgs bosons if there are more than 3
    }
...
void makeNtuplesHHH6bAllObjectsOptionalSel(TString inputFile, TString outputFile, TString modelPathAK4, TString modelPathFatJet, TString jetBranch = "JetPUPPI", TString fatJetBranch = "JetPUPPIAK8", TString selectionLevel = "4j") {
    ...
    for (Long64_t entry = 0; entry < allEntries; ++entry) {
        ...
        for(int i = 0; i < branchParticle->GetEntriesFast(); ++i) {
            ...
            processGenParticle(genparticle, i, data, branchParticle, higgs_count, higgs1_p4, higgs2_p4, higgs3_p4, bhadrons_info, motherMap);
        }
